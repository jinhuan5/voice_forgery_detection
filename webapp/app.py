...
class VoiceDetector:
    ...
    def extract_features(self, audio_path):
        ...
        try:
            ...
            if np.isscalar(tempo):
                ...
                # 如果tempo是数组，取第一个元素或平均值
                features.append(float(np.mean(tempo)) if len(tempo) > 0 else 0.0)
    ...
    def predict(self, audio_path):
        ...
        try:
            ...
            print(f"预测结果: prediction={prediction}, probability={probability}")
...
def health():
    ...
    return jsonify({
        ...
        'model_type': 'clean_data_model' if detector.model is not None else 'none',
...
def upload_file():
    ...
    try:
        ...
        # 创建频谱图
        spectrogram = create_spectrogram(filepath)

        # 清理上传的文件
        try:
            os.remove(filepath)
            print(f"临时文件已清理: {filepath}")
        except:
            pass
